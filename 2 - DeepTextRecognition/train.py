...
def train(opt):
    ...
    while(True):
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            ...
            projection_time = (elapsed_time / (iteration + 1)) * opt.num_iter
            time_cicle = elapsed_time/(iteration+1)

            if iteration == 0:
                projection_time /= 8.2
                time_cicle /= 8.2
            
            remain_time = projection_time - elapsed_time
...
def get_time(time):
    ...
    if time >= 86400:
        t += f'{(math.floor(time/86400)):0.0f}d '
        time = time % 86400
        
    if time >= 3600:
        t += f'{(math.floor(time/3600)):02.0f}:'
        time = time % 3600
    else:
        t += '00:'
        
    if time >= 60:
        t += f'{(math.floor(time/60)):02.0f}:'
        time = time % 60
    else:
        t += '00:'
